...
public class SWEA_1767_프로세서연결하기 {
	...
	static void dfs(int index, int coreCnt, int len) { //len은 간선의 길이 합
		if(coreCnt == list.size()) { //기저조건 : 증가되는 인덱스가 list의 사이즈만큼 되었을 때
			if(max < coreCnt) { //코어개수가 더 많아서 갱신해야 될 때
				max = coreCnt;
				min = len;
			}
			else if(max == coreCnt) { //코어개수가 같으면 길이 비교
				if(min > len) min = len;
			}
			return;
		}
